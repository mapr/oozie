...
copyExtraLib(){
  ...
  findAndCopyJar "$HADOOP_HOME" "$OOZIE_HOME/share/lib/sqoop" -name "hadoop-common*.jar" -not -name "*tests*.jar"
  findAndCopyJar "$MAPR_HOME" "$OOZIE_HOME/share/lib/sqoop" -name "zookeeper-jute*.jar" -not -name "*tests*.jar"
}
...
copyMaprLibs() {
  ...
  findAndCopyJar "$MAPR_HOME/lib" "${JETTY_LIB_DIR}" -name "bc-fips-*.jar" -not -name "*tests*.jar" 2> /dev/null
  findAndCopyJar "$MAPR_HOME/lib" "${JETTY_LIB_DIR}" -name "bctls-fips-*.jar" -not -name "*tests*.jar" 2> /dev/null
