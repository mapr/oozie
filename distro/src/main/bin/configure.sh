...
copyMaprLibs() {
  ...
  findAndCopyJar "${MAPR_HOME}/" "${JETTY_LIB_DIR}" -iname "bctls-fips*.jar" -not -name "*test*.jar" 2> /dev/null
  findAndCopyJar "${MAPR_HOME}/" "${JETTY_LIB_DIR}" -iname "bc-fips*.jar" -not -name "*test*.jar" 2> /dev/null
